...
# for training models
if TRAIN_MODELS:
    model = MLPClassifier(layers=[784, 256, 128, 64, 10], activation_function='relu', learning_rate=0.01, batch_size=64, num_epochs=20, optimizer='AdaGrad', weight_init='he', regularization=None)
    model = model.fit(X_train,y_train, Xtest=X_test, ytest=y_test, save_error=True)
    pickle.dump(model, open("Models/relu_adagrad_he.pkl", "wb"))
    print("Train Accuracy:",model.score(X_train,y_train))
    print("Tests Accuracy:",model.score(X_test,y_test))

    model = MLPClassifier(layers=[784, 256, 128, 64, 10], activation_function='relu', learning_rate=0.01, batch_size=64, num_epochs=20, optimizer='AdaGrad', weight_init='xavier', regularization=None)
    model = model.fit(X_train,y_train, Xtest=X_test, ytest=y_test, save_error=True)
    pickle.dump(model, open("Models/relu_adagrad_xavier.pkl", "wb"))
    print("Train Accuracy:",model.score(X_train,y_train))
    print("Tests Accuracy:",model.score(X_test,y_test))
